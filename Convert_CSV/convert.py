...
def convert_file(input_file):
    ...
    # Regex pattern
    pattern = re.compile(r'time="([\d\-:\s]+)"\s+type=([A-Z_]+)\s+package=([a-zA-Z0-9\.]+)')  # for com.apps

    csv_data = [['Timestamp', 'Activity Type', 'Application']]

    # Loop through lines - append time and duration
    for line in data:
    ...
    if not output_file:  # User canceled the save dialog
        return None
...
root.resizable(False, False)  # Allows resizing for testing, set to False for no resize
